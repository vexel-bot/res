...
export function BrainView() {
  ...
  return <div className="mx-auto w-full max-w-[1480px] p-6 2xl:p-10">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.24em] text-[#ff5c5c]"><BrainCircuit className="h-4 w-4" />Memória estratégica persistente</div><h1 className="mt-2 text-2xl font-semibold text-white">Memória da marca</h1><p className="mt-1 text-sm text-[#8f999f]">Fonte única de verdade consultada em toda ação de IA.</p></div><button onClick={save} className="flex items-center gap-2 rounded-lg bg-[#ff5c5c] px-4 py-2.5 text-[11px] font-bold text-[#14200e]">{saved ? <Check className="h-4 w-4" /> : <Save className="h-4 w-4" />}{saved ? 'Memória atualizada' : 'Salvar nova revisão'}</button></div>
    <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_280px]">
      <section className="grid gap-3 md:grid-cols-2">{fields.map(({ key, label, description, wide }) => <label key={key} className={`rounded-xl border border-white/[0.07] bg-[#182126] p-4 ${wide ? 'md:col-span-2' : ''}`}><span className="text-[11px] font-semibold text-white">{label}</span><span className="mt-0.5 block text-[9px] text-[#78848a]">{description}</span><textarea value={String(draft[key] ?? '')} onChange={(event) => setDraft((current) => ({ ...current, [key]: event.target.value }))} rows={wide ? 3 : 4} className="mt-3 w-full resize-none rounded-lg border border-white/[0.06] bg-black/25 p-3 text-[11px] leading-relaxed text-[#dbe0e2] outline-none focus:border-[#ff5c5c]/40" /></label>)}</section>
      <aside className="space-y-4">
        <div className="sticky top-24 rounded-xl border border-white/[0.07] bg-[#182126] p-5"><div className="flex items-center justify-between"><span className="text-[11px] font-semibold text-white">Completude</span><span className="text-lg font-semibold text-[#ff5c5c]">{brainCompleteness}%</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full bg-[#ff5c5c]" style={{ width: `${brainCompleteness}%` }} /></div><div className="mt-4 flex items-center gap-2 rounded-lg bg-[#ff5c5c]/[0.07] p-3 text-[9px] leading-relaxed text-[#b8c0c4]"><ShieldCheck className="h-5 w-5 shrink-0 text-[#ff5c5c]" />IA vinculada à revisão {brain.revision}. Nenhuma geração acontece sem este contexto.</div><h3 className="mt-5 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9da6ab]">Fontes conectadas</h3><div className="mt-3 space-y-2">{brain.sourceFiles.map((source) => <div key={source.id} className="flex items-center gap-2 rounded-lg border border-white/[0.05] bg-black/20 p-2.5"><FilePlus2 className="h-4 w-4 text-[#ff5c5c]" /><span className="min-w-0 truncate text-[9px] text-[#cbd1d4]">{source.name}</span></div>)}</div><button onClick={() => addBrainSource({ id: `source-${Date.now()}`, name: 'Novo material de referência', type: 'document', addedAt: new Date().toISOString() })} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border border-dashed border-white/10 py-2.5 text-[9px] text-[#9ba5aa] hover:border-[#ff5c5c]/35 hover:text-[#ff5c5c]"><FilePlus2 className="h-4 w-4" />Adicionar arquivo</button><div className="mt-3 flex items-center gap-2 text-[9px] text-[#758087]"><Link2 className="h-3.5 w-3.5" />{brain.sourceLinks.length} link conectado</div></div>
      </aside>
    </div>
  </div>;
}
