...
export function BrainView() {
  ...
  return <div className="mx-auto w-full max-w-[1480px] p-6 2xl:p-10">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.24em] text-[#8bd132]"><BrainCircuit className="h-4 w-4" />Memória estratégica persistente</div><h1 className="mt-2 text-2xl font-semibold text-white">Brain da marca</h1><p className="mt-1 text-sm text-[#8f999f]">Fonte única de verdade consultada em toda ação de IA.</p></div><button onClick={save} className="flex items-center gap-2 rounded-lg bg-[#8bd132] px-4 py-2.5 text-[11px] font-bold text-[#14200e]">{saved ? <Check className="h-4 w-4" /> : <Save className="h-4 w-4" />}{saved ? 'Brain atualizado' : 'Salvar nova revisão'}</button></div>
