...
export function WorkspaceView({ onNavigate }: { onNavigate: (tab: NavigationTab) => void }) {
  ...
  const modules = [
    { id: 'brain' as const, title: 'Brain da marca', detail: `${brainCompleteness}% estruturado`, icon: BrainCircuit },
    { id: 'strategy' as const, title: 'Estratégias', detail: `${active} campanha ativa`, icon: Target },
    { id: 'studio' as const, title: 'Studio', detail: `${posts.length} conteúdos no fluxo`, icon: Clapperboard },
  ...
  return <div className="mx-auto w-full max-w-[1480px] space-y-5 p-6 2xl:p-10">
    ...
    <section className="grid gap-3 md:grid-cols-4">
      {[['Campanhas ativas', active, Target], ['Conteúdos agendados', scheduled, CalendarDays], ['Pessoas no workspace', users.length, Users], ['Ativos de memória', assets.length + posts.length, FolderKanban]].map(([label, value, Icon]) => <div key={String(label)} className="rounded-xl border border-white/[0.07] bg-[#182126] p-4"><div className="flex items-center justify-between"><span className="text-[10px] text-[#9da6ab]">{String(label)}</span><Icon className="h-4 w-4 text-[#8bd132]" /></div><strong className="mt-4 block text-2xl font-medium text-white">{String(value)}</strong></div>)}
    </section>

    <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      <div className="rounded-xl border border-white/[0.07] bg-[#182126] p-5">
        <div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold text-white">Arquitetura do workspace</h2><p className="mt-1 text-[10px] text-[#8d979d]">Cada etapa alimenta a próxima sem duplicar contexto.</p></div><span className="rounded-full bg-[#8bd132]/10 px-2.5 py-1 text-[9px] text-[#8bd132]">Sincronizado</span></div>
      ...
      <div className="rounded-xl border border-white/[0.07] bg-[#182126] p-5"><h2 className="text-sm font-semibold text-white">Campanhas recentes</h2><div className="mt-4 space-y-3">{campaigns.slice(0, 4).map((campaign) => <button key={campaign.id} onClick={() => onNavigate('strategy')} className="w-full rounded-lg border border-white/[0.05] bg-black/20 p-3 text-left"><div className="flex justify-between gap-3"><span className="text-[11px] font-medium text-white">{campaign.name}</span><span className="text-[8px] uppercase text-[#8bd132]">{campaign.status}</span></div><p className="mt-1 line-clamp-2 text-[9px] text-[#899399]">{campaign.objective}</p></button>)}</div></div>
    </section>
  </div>;
}
