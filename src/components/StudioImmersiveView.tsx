  ...
  return (
    <div className="clicko-studio-immersive min-h-[calc(100vh-72px)] lg:grid lg:grid-cols-[232px_minmax(0,1fr)]">
      <aside className="clicko-studio-sidebar custom-scrollbar border-b border-white/[0.06] px-5 py-5 lg:sticky lg:top-[72px] lg:h-[calc(100vh-80px)] lg:overflow-y-auto lg:border-b-0 lg:border-r lg:px-4 lg:py-7">
        <div className="px-2">
          <div className="flex items-center gap-2 text-[9px] font-medium uppercase tracking-[0.22em] text-[#8bd132]"><Sparkles className="h-3.5 w-3.5" />Creative OS</div>
          ...
          {modules.map(({ id, label, short, icon: Icon }, index) => {
            ...
            return (
              <button key={id} type="button" onClick={() => setMode(id)} aria-pressed={active} className={`clicko-interactive-surface group relative flex min-w-[168px] items-center gap-3 rounded-md border border-transparent px-2.5 py-2.5 text-left lg:w-full lg:min-w-0 ${active ? 'bg-white/[0.055]' : 'hover:bg-white/[0.025]'}`}>
                {active && <span className="absolute inset-y-2 left-0 w-px bg-[#8bd132]" />}
                <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-md border ${active ? 'border-[#8bd132]/25 bg-[#8bd132]/10 text-[#8bd132]' : 'border-white/[0.06] text-[#68747b] group-hover:text-white'}`}><Icon className="h-3.5 w-3.5" /></span>
        ...
        <div className="mt-6 border-t border-white/[0.06] px-2 pt-5">
          ...
          <select value={activeCampaign?.id || ''} onChange={(event) => setActiveCampaignId(event.target.value || undefined)} className="mt-2 h-9 w-full rounded-md border border-white/[0.07] bg-transparent px-2.5 text-[9px] text-[#b8c0c4] outline-none focus:border-[#8bd132]/40">
            <option value="">Sem campanha</option>
            {campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}
          </select>
          <div className="mt-4 flex items-center gap-2 text-[8px] text-[#5f6b72]"><BrainCircuit className="h-3.5 w-3.5 text-[#8bd132]" /><span>Memória da marca · revisão {brain.revision}</span></div>
      ...
      <main className="min-w-0">
        <header className="clicko-studio-workspace-header flex flex-col gap-4 border-b border-white/[0.06] px-6 py-6 md:flex-row md:items-end md:justify-between 2xl:px-9 2xl:py-8">
          <div>
            <span className="text-[9px] font-medium uppercase tracking-[0.2em] text-[#8bd132]">{description.eyebrow}</span>
          ...
          <div className="flex items-center gap-2 text-[8px] uppercase tracking-[0.16em] text-[#556168]"><span className="h-1.5 w-1.5 rounded-full bg-[#8bd132]" />Contexto sincronizado</div>
        </header>
