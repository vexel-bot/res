...
export const SettingsView: React.FC = () => {
  ...
  const renderCompany = () => <div className="space-y-4"><SettingsCard title="Dados da empresa" description="Informações usadas em contas, relatórios e integrações."><div className="grid gap-4 sm:grid-cols-2"><Field label="Razão social" defaultValue="Clicko AI Studios Tecnologia Ltda." /><Field label="Nome da marca" defaultValue="Clicko AI Studios" /><Field label="CNPJ" defaultValue="00.000.000/0001-00" /><Field label="Site" defaultValue="https://clickostudio.com" /><Field label="E-mail corporativo" defaultValue="contato@clickostudio.com" /><Field label="Telefone" defaultValue="+55 (11) 99999-0000" /></div></SettingsCard><SettingsCard title="Identidade operacional"><div className="grid gap-4 sm:grid-cols-2"><SelectField label="Idioma da empresa" defaultValue="pt-BR"><option value="pt-BR">Português (Brasil)</option><option value="en-US">English (US)</option></SelectField><SelectField label="Fuso horário" defaultValue="America/Sao_Paulo"><option value="America/Sao_Paulo">São Paulo · GMT-3</option><option value="America/New_York">New York · GMT-4</option></SelectField></div><p className="mt-4 rounded-lg bg-[#8bd132]/[0.06] p-3 text-[9px] leading-relaxed text-[#9ca7ac]">Brain, Strategy, equipe e contas conectadas utilizam estes dados como referência compartilhada.</p></SettingsCard></div>;

  const renderPlatform = () => <div className="space-y-4"><div className="grid gap-4 lg:grid-cols-2"><SettingsCard title="Tema e interface"><div className="space-y-4"><SelectField label="Tema" value={settings.appearance.theme} onChange={(event) => patchSettings('appearance', { theme: event.target.value as PersonalSettings['appearance']['theme'] })}><option value="dark">Escuro</option><option value="light">Claro</option><option value="system">Seguir sistema</option></SelectField><SelectField label="Densidade" value={settings.appearance.layout} onChange={(event) => patchSettings('appearance', { layout: event.target.value as PersonalSettings['appearance']['layout'] })}><option value="comfortable">Confortável</option><option value="compact">Compacta</option></SelectField><ToggleRow title="Animações" description="Transições e microinterações" checked={settings.appearance.animations} onChange={(value) => patchSettings('appearance', { animations: value })} /></div></SettingsCard><SettingsCard title="APIs da plataforma"><div className="space-y-3"><div className="flex items-center justify-between rounded-lg border border-white/[0.06] bg-[#11191d] p-3"><div><span className="block text-[10px] text-white">Gemini AI</span><span className="text-[8px] text-[#748087]">Processamento server-side protegido</span></div><span className="rounded-full bg-[#8bd132]/10 px-2 py-1 text-[8px] text-[#8bd132]">Ativa</span></div><div className="flex items-center justify-between rounded-lg border border-white/[0.06] bg-[#11191d] p-3"><div><span className="block text-[10px] text-white">Webhooks</span><span className="text-[8px] text-[#748087]">Eventos para sistemas externos</span></div><button className="text-[8px] text-[#8bd132]">Configurar</button></div></div></SettingsCard></div><SettingsCard title="Integrações gerais" description="Serviços compartilhados por todos os módulos."><div className="grid gap-3 sm:grid-cols-3">{[['Armazenamento', 'Conectado'], ['Analytics', 'Conectado'], ['E-mail transacional', 'Conectado']].map(([name, status]) => <div key={name} className="flex items-center gap-3 rounded-lg border border-white/[0.06] bg-[#11191d] p-3"><MonitorSmartphone className="h-4 w-4 text-[#8bd132]" /><div><span className="block text-[9px] text-white">{name}</span><span className="text-[8px] text-[#8bd132]">{status}</span></div></div>)}</div></SettingsCard></div>;
