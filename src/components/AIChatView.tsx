...
export function AIChatView({ onNavigate }: { onNavigate?: (tab: NavigationTab) => void }) {
  ...
  return <div className="mx-auto grid h-[calc(100vh-82px)] w-full max-w-[1500px] grid-cols-1 gap-4 p-5 lg:grid-cols-[260px_minmax(0,1fr)] lg:p-7">
    <aside className="hidden overflow-hidden rounded-xl border border-white/[0.07] bg-[#141b1f] lg:flex lg:flex-col">
      <div className="border-b border-white/[0.06] p-4"><div className="flex items-center gap-2 text-[11px] font-semibold text-white"><Clock3 className="h-4 w-4 text-[#ff5c5c]" />Histórico</div><p className="mt-1 text-[8px] text-[#758087]">Memória compartilhada com o assistente flutuante.</p></div>
    ...
    <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-white/[0.07] bg-[#141b1f]">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] p-4">
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#ff5c5c]/10">
            <Bot className="h-5 w-5 text-[#ff5c5c]" />
          </span>
          <div>
        ...
        <div className="flex flex-wrap gap-2">{activeClient && <span className="max-w-[170px] truncate rounded-full bg-[#ff7a00]/[0.07] px-2.5 py-1.5 text-[8px] text-[#ff9a3d]">Cliente · {activeClient.name}</span>}<span className="flex items-center gap-1.5 rounded-full bg-[#ff5c5c]/[0.07] px-2.5 py-1.5 text-[8px] text-[#ff8a8a]"><BrainCircuit className="h-3 w-3" />Memória rev. {brain.revision}</span>{activeCampaign && <span className="max-w-[190px] truncate rounded-full bg-white/[0.04] px-2.5 py-1.5 text-[8px] text-[#aab3b7]">{activeCampaign.name}</span>}</div></header>
      <div className="custom-scrollbar flex-1 overflow-y-auto p-4 md:p-6"><div className="mx-auto max-w-3xl space-y-4">{messages.map((message) => <div key={message.id} className={`group flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[84%] rounded-2xl px-4 py-3 ${message.role === 'user' ? 'bg-[#ff5c5c] text-white' : 'border border-white/[0.06] bg-black/25 text-[#d7dddf]'}`}><p className="whitespace-pre-wrap text-[10px] leading-relaxed">{message.content}</p>{message.role === 'assistant' && message.actions?.length ? <div className="mt-3 flex flex-wrap gap-2 border-t border-white/[0.06] pt-3">{message.actions.map((action) => <button key={`${message.id}-${action.label}`} onClick={() => onNavigate?.(action.tab)} className="rounded-lg border border-[#ff5c5c]/20 bg-[#ff5c5c]/[0.06] px-2.5 py-1.5 text-[8px] font-semibold text-[#ff8a8a]">{action.label}</button>)}</div> : null}<div className={`mt-2 flex items-center justify-between gap-4 text-[7px] ${message.role === 'user' ? 'text-white/60' : 'text-[#637076]'}`}><span>{new Date(message.createdAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })} · {navigationLabel[message.module]}</span>{message.role === 'assistant' && <button onClick={() => toggleFavorite(message.id)} aria-label="Favoritar resposta"><Heart className={`h-3 w-3 ${message.favorite ? 'fill-[#ff5c5c] text-[#ff5c5c]' : ''}`} /></button>}</div></div></div>)}{loading && <div className="flex items-center gap-2 text-[9px] text-[#ff7a00]"><Sparkles className="h-4 w-4 animate-pulse" />Analisando cliente, campanha e contexto da plataforma…</div>}</div></div>
      <div className="border-t border-white/[0.06] p-4"><div className="mx-auto max-w-3xl"><div className="mb-3 flex gap-2 overflow-x-auto">{starters.map(({ icon: Icon, label, prompt }) => <button key={label} onClick={() => setInput(prompt)} className="flex shrink-0 items-center gap-1.5 rounded-full border border-white/[0.06] bg-white/[0.025] px-3 py-1.5 text-[8px] text-[#9ea8ad] hover:border-[#ff5c5c]/25 hover:text-[#ff5c5c]"><Icon className="h-3 w-3" />{label}</button>)}</div><div className="flex items-end gap-2 rounded-xl border border-white/[0.08] bg-black/25 p-2 focus-within:border-[#ff5c5c]/30"><textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submit(); } }} rows={2} placeholder="Converse com a IA usando todo o contexto da sua operação…" className="min-h-10 flex-1 resize-none bg-transparent px-2 py-1.5 text-[10px] text-white outline-none" /><button onClick={submit} disabled={!input.trim() || loading} className="grid h-9 w-9 place-items-center rounded-lg bg-[#ff5c5c] text-[#14200e] disabled:opacity-35"><Send className="h-4 w-4" /></button></div></div></div>
    </section>
  </div>;
}
