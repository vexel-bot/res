...
import { Bot, BrainCircuit, Clock3, Heart, Lightbulb, Megaphone, Search, Send, Sparkles, Trash2 } from 'lucide-react';
import { useAIChat } from '../context/AIChatContext';
import { useOperations } from '../context/OperationsContext';

const starters = [
  { icon: Lightbulb, label: 'Brainstorm', prompt: 'Gere ideias de conteúdo alinhadas ao posicionamento da marca.' },
  { icon: Megaphone, label: 'Marketing', prompt: 'Crie uma estratégia de marketing para a campanha ativa.' },
  { icon: Search, label: 'SEO', prompt: 'Sugira pautas e palavras-chave para melhorar nosso SEO.' },
  { icon: Sparkles, label: 'Revisão', prompt: 'Quais melhorias você recomenda para os conteúdos atuais?' },
];
...
export function AIChatView() {
  ...
  return <div className="mx-auto grid h-[calc(100vh-82px)] w-full max-w-[1500px] grid-cols-1 gap-4 p-5 lg:grid-cols-[260px_minmax(0,1fr)] lg:p-7">
    ...
    <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-white/[0.07] bg-[#141b1f]">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] p-4"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#8bd132]/10"><Bot className="h-5 w-5 text-[#8bd132]" /></span><div><h1 className="text-sm font-semibold text-white">AI Chat</h1><p className="text-[8px] text-[#758087]">Conversa livre, criação, estratégia, SEO e planejamento</p></div></div><div className="flex gap-2"><span className="flex items-center gap-1.5 rounded-full bg-[#8bd132]/[0.07] px-2.5 py-1.5 text-[8px] text-[#8bd132]"><BrainCircuit className="h-3 w-3" />Brain rev. {brain.revision}</span>{activeCampaign && <span className="max-w-[190px] truncate rounded-full bg-white/[0.04] px-2.5 py-1.5 text-[8px] text-[#aab3b7]">{activeCampaign.name}</span>}</div></header>
      <div className="custom-scrollbar flex-1 overflow-y-auto p-4 md:p-6"><div className="mx-auto max-w-3xl space-y-4">{messages.map((message) => <div key={message.id} className={`group flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[84%] rounded-2xl px-4 py-3 ${message.role === 'user' ? 'bg-[#8bd132] text-[#14200e]' : 'border border-white/[0.06] bg-black/25 text-[#d7dddf]'}`}><p className="whitespace-pre-wrap text-[10px] leading-relaxed">{message.content}</p><div className={`mt-2 flex items-center justify-between gap-4 text-[7px] ${message.role === 'user' ? 'text-[#28411a]' : 'text-[#637076]'}`}><span>{new Date(message.createdAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })} · {message.module}</span>{message.role === 'assistant' && <button onClick={() => toggleFavorite(message.id)} aria-label="Favoritar resposta"><Heart className={`h-3 w-3 ${message.favorite ? 'fill-[#8bd132] text-[#8bd132]' : ''}`} /></button>}</div></div></div>)}{loading && <div className="flex items-center gap-2 text-[9px] text-[#8bd132]"><Sparkles className="h-4 w-4 animate-pulse" />Analisando o contexto da plataforma…</div>}</div></div>
