...
export const SubscriptionView: React.FC = () => {
  const { plans, subscription, workspace, users, changePlan, loading, environmentMode } = useGovernance();
  ...
  const visiblePlans = environmentMode === 'personal' ? plans.filter((plan) => plan.id === 'solo') : plans;
  ...
  const confirmPlan = async () => {
    ...
    const targetIndex = plans.findIndex((plan) => plan.id === selectedPlan);
    if (targetIndex > currentIndex) {
      const target = plans[targetIndex];
      const shown = openOffer({
        context: 'upgrade', targetPlanId: selectedPlan, source: 'subscription.plan-selector',
        reason: `Você escolheu o plano ${target.name}. Veja apenas as diferenças relevantes antes de continuar.`,
      });
      if (shown) return;
    }
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] space-y-5 p-5 md:p-7">
      <header className="border-b border-white/[0.06] pb-5"><div className="mb-2 flex items-center gap-2 text-[9px] uppercase tracking-[0.2em] text-[#7C7C7C]"><ShieldCheck className="h-3.5 w-3.5" /> Acesso exclusivo do administrador</div><h1 className="text-[22px] font-semibold tracking-[-0.035em] text-white">Assinatura e capacidade</h1><p className="mt-1 text-[10px] text-[#7f888d]">Gerencie plano, assentos e cobrança do ambiente de trabalho.</p></header>
