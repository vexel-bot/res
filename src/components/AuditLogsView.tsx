...
export const AuditLogsView: React.FC = () => {
  ...
  return <div className="mx-auto w-full max-w-[1400px] space-y-5 p-5 md:p-7">
    <header className="flex flex-wrap items-end justify-between gap-4 border-b border-white/[0.06] pb-5">
      <div><div className="mb-2 flex items-center gap-2 text-[9px] uppercase tracking-[0.2em] text-[#ff5c5c]"><ShieldCheck className="h-3.5 w-3.5" />Segurança e rastreabilidade</div><h1 className="text-[22px] font-semibold tracking-[-0.035em] text-white">Registros de auditoria</h1><p className="mt-1 text-[10px] text-[#7f888d]">Registro pesquisável de ações, módulos, usuários e empresas.</p></div>
      <button onClick={exportCsv} className="flex items-center gap-2 rounded-lg border border-white/[0.07] bg-[#182126] px-4 py-2.5 text-[9px] text-[#c7ced1] hover:border-[#ff5c5c]/25 hover:text-[#ff5c5c]"><Download className="h-3.5 w-3.5" />Exportar CSV</button>
    </header>
    ...
    <section className="overflow-hidden rounded-xl border border-white/[0.065] bg-[#111111]">
      <div className="flex items-center justify-between border-b border-white/[0.055] p-4"><div className="flex items-center gap-2 text-[10px] font-medium text-white"><Activity className="h-4 w-4 text-[#ff5c5c]" />Atividade recente</div><span className="text-[8px] text-[#636b6f]">{visible.length} eventos · {workspace?.name}</span></div>
      {loading && auditLogs.length === 0 ? <div className="p-8 text-[10px] text-[#71797d]">Carregando eventos…</div> : <div className="divide-y divide-white/[0.045]">
        {visible.map((entry) => <div key={entry.id} className="grid gap-3 p-4 sm:grid-cols-[36px_160px_110px_1fr_auto] sm:items-center">
          <span className="grid h-8 w-8 place-items-center rounded-lg bg-white/[0.045] text-[#ff5c5c]"><FileSearch className="h-4 w-4" /></span>
          ...
          <span className="w-fit rounded-full bg-[#ff5c5c]/[0.07] px-2 py-1 text-[7px] text-[#ff5c5c]">{localizeAuditToken(entry.resource.split(':')[0])}</span>
