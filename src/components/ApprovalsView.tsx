  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] p-5 md:p-7">
      <header className="mb-5 flex flex-col gap-4 border-b border-white/[0.06] pb-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex rounded-lg border border-white/[0.07] bg-[#111] p-1"><button onClick={() => setFilter('queue')} className={`rounded-md px-3 py-2 text-[9px] font-semibold ${filter === 'queue' ? 'bg-[#8bd132] text-[#0e170a]' : 'text-[#858d91]'}`}>Fila ativa</button><button onClick={() => setFilter('all')} className={`rounded-md px-3 py-2 text-[9px] font-semibold ${filter === 'all' ? 'bg-[#8bd132] text-[#0e170a]' : 'text-[#858d91]'}`}>Histórico completo</button></div>
      </header>
