  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] p-5 md:p-7">
      <header className="mb-5 flex flex-col gap-4 border-b border-white/[0.06] pb-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex rounded-lg border border-white/[0.07] bg-[#111] p-1"><button onClick={() => setFilter('queue')} className={`rounded-md px-3 py-2 text-[9px] font-semibold ${filter === 'queue' ? 'bg-[#ff5c5c] text-[#0e170a]' : 'text-[#858d91]'}`}>Fila ativa</button><button onClick={() => setFilter('all')} className={`rounded-md px-3 py-2 text-[9px] font-semibold ${filter === 'all' ? 'bg-[#ff5c5c] text-[#0e170a]' : 'text-[#858d91]'}`}>Histórico completo</button></div>
      </header>
      ...
      <div className="grid min-h-[650px] overflow-hidden rounded-xl border border-white/[0.065] bg-[#111111] lg:grid-cols-[340px_minmax(0,1fr)]">
        ...
        {selected ? <main className="min-w-0"><div className="grid gap-5 p-5 xl:grid-cols-[minmax(0,1fr)_300px]"><div className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-[8px] uppercase tracking-[0.14em] text-[#697277]">{selected.platform} · {selected.format}</div><h2 className="mt-2 max-w-[760px] text-[18px] font-semibold leading-snug text-white">{selected.title}</h2><p className="mt-2 text-[9px] text-[#777f84]">Criado por <span className="text-[#B8B8B8]">{selected.authorName}</span> em {formatDateTime(selected.createdAt)}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1.5 text-[8px] ${stageMeta[selected.stage].className}`}>{stageMeta[selected.stage].label}</span></div>
              ...
              <aside className="space-y-4"><div className="rounded-xl border border-white/[0.065] bg-[#0E0E0E] p-4"><h3 className="text-[10px] font-medium text-white">Decisão do administrador</h3><p className="mt-1 text-[8px] leading-relaxed text-[#687175]">A publicação só será liberada depois da aprovação registrada.</p><div className="mt-4 space-y-2"><button onClick={() => void act('approve')} disabled={selected.stage === 'approved' || selected.stage === 'published'} className="flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-[#ff5c5c] hover:bg-[#9be24d] text-[9px] font-bold text-[#0e170a] shadow-md shadow-[#ff5c5c]/20 transition-all disabled:opacity-30"><Check className="h-3.5 w-3.5" />Aprovar conteúdo</button><button onClick={() => void act('request_changes', true)} disabled={!comment.trim()} className="flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-amber-400/15 bg-amber-400/[0.05] text-[8px] text-amber-200 disabled:opacity-30"><Wrench className="h-3.5 w-3.5" />Solicitar ajustes</button><button onClick={() => void act('reject', true)} disabled={!comment.trim()} className="flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-red-400/15 bg-red-400/[0.04] text-[8px] text-red-300 disabled:opacity-30"><ThumbsDown className="h-3.5 w-3.5" />Reprovar</button></div></div>
                <div className="rounded-xl border border-white/[0.065] bg-[#0E0E0E] p-4"><h3 className="text-[10px] font-medium text-white">Publicação</h3><div className="mt-3"><label className="text-[8px] text-[#717a7e]">Data e horário</label><input type="datetime-local" value={scheduleAt} onChange={(event) => setScheduleAt(event.target.value)} className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.07] bg-[#0B0B0B] px-2 text-[8px] text-[#c6cbcd] outline-none" /></div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => void act('schedule')} disabled={selected.stage !== 'approved'} className="flex h-9 items-center justify-center gap-1.5 rounded-lg border border-white/[0.08] text-[8px] text-[#B8B8B8] disabled:opacity-25"><CalendarClock className="h-3.5 w-3.5" />Agendar</button><button onClick={() => void act('publish')} disabled={selected.stage !== 'approved'} className="flex h-9 items-center justify-center gap-1.5 rounded-lg bg-[#ff5c5c] text-[8px] font-bold text-[#0e170a] disabled:opacity-25"><Send className="h-3.5 w-3.5" />Publicar agora</button></div>{selected.stage !== 'approved' && <p className="mt-2 flex items-start gap-1.5 text-[7px] leading-relaxed text-[#636b6f]"><X className="mt-0.5 h-3 w-3 shrink-0" />Bloqueado até a aprovação do administrador.</p>}</div>
                ...
                <div className="rounded-xl border border-white/[0.065] bg-[#0E0E0E] p-4"><div className="flex items-center justify-between"><span className="text-[10px] font-medium text-white">Versões</span><span className="text-[8px] text-[#ff5c5c]">{selected.versions?.length || 1} registrada(s)</span></div><div className="mt-3 space-y-2">{(selected.versions || [{ id: 'initial', number: 1, label: 'Versão inicial', author: selected.authorName, createdAt: selected.createdAt }]).map((version) => <div key={version.id} className="rounded-lg border border-white/[0.05] bg-black/20 p-2.5"><div className="flex justify-between"><span className="text-[8px] text-[#d0d5d7]">v{version.number} · {version.label}</span><span className="text-[7px] text-[#626c70]">{formatDateTime(version.createdAt)}</span></div><p className="mt-1 text-[7px] text-[#707a7f]">{version.author}</p></div>)}</div></div>
